...
predict_one_chunk <- function(chunk_id, seqs, out_path) {
  chunk_start <- Sys.time()
  if (file.exists(out_path) && file.info(out_path)$size > 0) {
    return(data.frame(chunk_id = chunk_id, status = "SKIPPED", seconds = 0, rows = length(seqs)))
  }

  input_list <- strsplit(seqs, "", fixed = TRUE)
  names(input_list) <- sprintf("unique_%d_%05d", chunk_id, seq_along(seqs))
  result <- tryCatch({
    ...
    data.frame(
      ...
      AmyloGram_Pred = paste0("CHUNK_ERROR: ", conditionMessage(e)),
      stringsAsFactors = FALSE
    )
  })
  ...
  data.frame(chunk_id = chunk_id, status = "DONE", seconds = elapsed, rows = nrow(result))
}

dir.create(dirname(output_file), recursive = TRUE, showWarnings = FALSE)
dir.create(chunk_dir, recursive = TRUE, showWarnings = FALSE)

cores <- get_env_int("AMYLOGRAM_CORES", max(1, detectCores() - 1))
chunk_size <- get_env_int("AMYLOGRAM_CHUNK_SIZE", 250)
...
log_msg("cores=%d chunk_size=%d", cores, chunk_size)
...
chunk_stats <- mclapply(seq_len(chunk_count), function(i) {
  ...
  stats <- predict_one_chunk(i, unique_seqs[idx], chunk_paths[i])
  log_msg("chunk %d/%d %s rows=%d seconds=%.2f", i, chunk_count, stats$status, stats$rows, stats$seconds)
  stats
}, mc.cores = cores, mc.preschedule = FALSE)
chunk_stats <- do.call(rbind, chunk_stats)

log_msg("combining chunk predictions")
...
summary <- data.frame(
  ...
  value = c(
    ...
    sum(grepl("^CHUNK_ERROR", expanded$AmyloGram_Pred)),
...
log_msg("DONE runtime_minutes=%.2f", elapsed_mins)
